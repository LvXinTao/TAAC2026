...
class AutoNSTokenizer(nn.Module):
    """AutoToken-style NS Tokenizer: data-driven feature grouping.

    Learns a selection matrix W that dynamically selects and weights features
    per token via Top-k + Softmax, making the whole process differentiable.

    When ns_groups is provided, the group structure is used for W warm-start.

    Reference: MTMixAtt paper, Section 3.2 AutoToken.
    """
    ...
    def __init__(
        ...
        n_f = len(feature_specs)

        if top_k <= 0:
            top_k = math.ceil(n_f / num_ns_tokens)
        top_k = max(1, min(top_k, n_f))
        self.top_k = top_k
        ...
        self.W = nn.Parameter(torch.zeros(num_ns_tokens, n_f))
        ...
        self.token_projs = nn.ModuleList([
            nn.Sequential(
                nn.Linear(top_k * emb_dim, d_model),
                nn.LayerNorm(d_model),
            )
            for _ in range(num_ns_tokens)
        ])
        ...
        logging.info(
            f"AutoNSTokenizer: {n_f} fids, top_k={top_k}, "
            f"num_ns_tokens={num_ns_tokens}, temperature={temperature}, "
            f"warm_start_groups={len(groups) if groups else 0}"
        )
    ...
    def forward(self, int_feats: torch.Tensor) -> torch.Tensor:
        ...
        for fid_idx in range(len(self.feature_specs)):
            ...
            emb_real_idx = self._emb_index[fid_idx]
            if emb_real_idx == -1:
                fid_emb = int_feats.new_zeros(B, self.emb_dim)
            else:
                emb_layer = self.embs[emb_real_idx]
                if length == 1:
                    fid_emb = emb_layer(int_feats[:, offset].long())
                    ...
                    fid_emb = (emb_all * mask).sum(dim=1) / count
            X.append(fid_emb)
        X = torch.stack(X, dim=1)  # (B, n_f, emb_dim)

        tokens = []
        for g in range(self.num_ns_tokens):
            w_g = self.W[g]  # (n_f,)
            topk_vals, topk_idx = w_g.topk(self.top_k)
            weights = F.softmax(topk_vals / self.temperature, dim=0)
            selected = X[:, topk_idx, :]  # (B, k, emb_dim)
            weighted = selected * weights.unsqueeze(0).unsqueeze(-1)
            flat = weighted.reshape(B, self.top_k * self.emb_dim)
            token = F.silu(self.token_projs[g](flat)).unsqueeze(1)
            tokens.append(token)

        return torch.cat(tokens, dim=1)  # (B, num_ns_tokens, d_model)


class UserDenseUEPairProjector(nn.Module):
    """Builds one user dense token from UE dense fids and weighted int-dense pairs.

    UE (User Experience) features are dense-only user vectors (e.g. fid 61, 87)
    that are projected directly.  Int-dense pair features (e.g. fid 62-66) have
    both int and dense representations; the dense values weight the int embeddings
    via a weighted-sum pooling before projection.
    """
    ...
    def __init__(
        ...
        int_by_fid = {
            int(fid): (int(vs), int(offset), int(length))
            for fid, (vs, offset, length) in zip(int_feature_ids, int_feature_specs)
        }
        dense_by_fid = {
            int(fid): (int(offset), int(length))
            for fid, offset, length in dense_feature_specs
        }

        self.ue_fids: List[int] = []
        self.ue_specs: List[Tuple[int, int]] = []
        for fid in ue_fids:
            fid = int(fid)
            if fid not in dense_by_fid:
                logging.warning(
                    f"user_dense_ue_fids contains fid={fid}, but it is missing "
                    "from user_dense schema; skipping it."
                )
                continue
            self.ue_fids.append(fid)
            self.ue_specs.append(dense_by_fid[fid])

        ue_dim = sum(length for _, length in self.ue_specs)
        self.ue_proj = (
            nn.Sequential(nn.Linear(ue_dim, d_model), nn.LayerNorm(d_model))
            if ue_dim > 0 else None
        )

        self.pair_fids: List[int] = []
        self.int_pair_specs: List[Tuple[int, int, int]] = []
        self.dense_pair_specs: List[Tuple[int, int]] = []
        embs = []
        for fid in pair_fids:
            fid = int(fid)
            if fid in self.ue_fids:
                continue
            if fid not in int_by_fid or fid not in dense_by_fid:
                logging.warning(
                    f"user_int_dense_pair_fids contains fid={fid}, but it is "
                    "missing from user_int or user_dense schema; skipping it."
                )
                continue
            ...
            if int_length != dense_length:
                logging.warning(
                    f"user_int_dense_pair_fids contains fid={fid}, but int_length={int_length} "
                    f"!= dense_length={dense_length}; skipping aligned weighted pair."
                )
                continue
            if int(vs) <= 0 or (emb_skip_threshold > 0 and int(vs) > emb_skip_threshold):
                logging.warning(
                    f"user_int_dense_pair_fids contains fid={fid}, but vocab_size={vs} "
                    "is unavailable or skipped by emb_skip_threshold; skipping it."
                )
                continue
            self.pair_fids.append(fid)
            self.int_pair_specs.append((int(vs), int_offset, int_length))
            self.dense_pair_specs.append((dense_offset, dense_length))
            embs.append(nn.Embedding(int(vs) + 1, emb_dim, padding_idx=0))

        self.embs = nn.ModuleList(embs)
        self.pair_proj = (
            nn.Sequential(
                nn.Linear(len(self.pair_fids) * emb_dim, d_model),
                nn.LayerNorm(d_model),
            )
            if self.pair_fids else None
        )

        fusion_parts = int(self.ue_proj is not None) + int(self.pair_proj is not None)
        self.out_proj = (
            nn.Sequential(nn.Linear(fusion_parts * d_model, d_model), nn.LayerNorm(d_model))
            if fusion_parts > 1 else None
        )

        logging.info(
            f"UserDenseUEPairProjector: ue_fids={self.ue_fids}, "
            f"pair_fids={self.pair_fids}"
        )

    @property
    def is_active(self) -> bool:
        return self.ue_proj is not None or self.pair_proj is not None

    def forward(self, int_feats: torch.Tensor, dense_feats: torch.Tensor) -> torch.Tensor:
        parts = []
        if self.ue_proj is not None:
            ue_slices = [
                dense_feats[:, offset:offset + length]
                for offset, length in self.ue_specs
            ]
            parts.append(F.silu(self.ue_proj(torch.cat(ue_slices, dim=-1))))

        if self.pair_proj is not None:
            pair_embs = []
            for emb_layer, (_, int_offset, int_length), (dense_offset, dense_length) in zip(
                self.embs, self.int_pair_specs, self.dense_pair_specs
            ):
                vals = int_feats[:, int_offset:int_offset + int_length].long()
                dense_slice = dense_feats[:, dense_offset:dense_offset + dense_length]
                emb_all = emb_layer(vals)
                mask = (vals != 0).to(emb_all.dtype).unsqueeze(-1)
                weights = dense_slice.to(emb_all.dtype).unsqueeze(-1) * mask
                denom = weights.abs().sum(dim=1).clamp(min=1e-6)
                pair_embs.append((emb_all * weights).sum(dim=1) / denom)
            parts.append(F.silu(self.pair_proj(torch.cat(pair_embs, dim=-1))))

        if len(parts) == 1:
            return parts[0]
        return F.silu(self.out_proj(torch.cat(parts, dim=-1)))


def _normalize_fid_list(fids: Optional[Union[str, List[int], Tuple[int, ...]]]) -> List[int]:
    """Normalize comma-separated or list-like fid config into sorted unique ints."""
    if fids is None:
        return []
    if isinstance(fids, str):
        raw_values = [x.strip() for x in fids.split(',') if x.strip()]
    else:
        raw_values = list(fids)
    out: List[int] = []
    seen = set()
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        user_dense_ue_fids: Optional[Union[str, List[int]]] = None,
        user_int_dense_pair_fids: Optional[Union[str, List[int]]] = None,
        ...
        # AutoToken parameters (only used when ns_tokenizer_type='auto')
        auto_top_k: int = 10,
        auto_temperature: float = 1.0,
    ) -> None:
        super().__init__()
        ...
        self.user_dense_ue_fids = _normalize_fid_list(user_dense_ue_fids)
        self.user_int_dense_pair_fids = _normalize_fid_list(user_int_dense_pair_fids)
        ...
        if ns_tokenizer_type == 'group':
            ...
            # AutoToken: data-driven feature grouping via learnable Top-k selection
            if user_ns_tokens <= 0:
                user_ns_tokens = len(user_ns_groups)
            if item_ns_tokens <= 0:
                item_ns_tokens = len(item_ns_groups)
        ...
        if self.has_user_dense:
            self.user_dense_ue_pair_projector = None
            if (self.user_dense_ue_fids and self.user_int_dense_pair_fids
                    and user_int_feature_ids and user_dense_feature_specs):
                ue_pair_projector = UserDenseUEPairProjector(
                    int_feature_specs=user_int_feature_specs,
                    int_feature_ids=user_int_feature_ids,
                    dense_feature_specs=user_dense_feature_specs,
                    ue_fids=self.user_dense_ue_fids,
                    pair_fids=self.user_int_dense_pair_fids,
                    emb_dim=emb_dim,
                    d_model=d_model,
                    emb_skip_threshold=emb_skip_threshold,
                )
                if ue_pair_projector.is_active:
                    self.user_dense_ue_pair_projector = ue_pair_projector
                else:
                    logging.warning(
                        "UserDenseUEPairProjector has no active UE or pair fids; "
                        "falling back to plain user dense projection."
                    )
            elif self.user_dense_ue_fids or self.user_int_dense_pair_fids:
                logging.warning(
                    "user_dense_ue_fids/user_int_dense_pair_fids were set, but schema "
                    "metadata is unavailable; falling back to plain user dense projection."
                )
            if self.user_dense_ue_pair_projector is None:
    ...
    def _init_params(self) -> None:
        ...
        if self.user_dense_ue_pair_projector is not None:
            for emb in self.user_dense_ue_pair_projector.embs:
                nn.init.xavier_normal_(emb.weight.data)
                emb.weight.data[0, :] = 0
    ...
    def reinit_high_cardinality_params(
        ...
        if self.user_dense_ue_pair_projector is not None:
            projector = self.user_dense_ue_pair_projector
            for i, (vs, _, _) in enumerate(projector.int_pair_specs):
                emb = projector.embs[i]
                if int(vs) > cardinality_threshold:
                    nn.init.xavier_normal_(emb.weight.data)
                    emb.weight.data[0, :] = 0
                    reinit_ptrs.add(emb.weight.data_ptr())
    ...
    def _make_user_dense_token(
        ...
        """Constructs the user dense token, using UE projector if available."""
        if self.user_dense_ue_pair_projector is not None:
            user_dense_tok = self.user_dense_ue_pair_projector(
