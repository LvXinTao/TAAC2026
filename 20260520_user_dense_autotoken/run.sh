...
# ---- Active config: Auto NST tokenizer (data-driven feature grouping) ----
USER_DENSE_UE_FIDS="${USER_DENSE_UE_FIDS:-61,87,89,90,91}"
USER_INT_DENSE_PAIR_FIDS="${USER_INT_DENSE_PAIR_FIDS:-62,63,64,65,66}"
AUTO_TOP_K="${AUTO_TOP_K:-10}"
AUTO_TEMPERATURE="${AUTO_TEMPERATURE:-1.0}"
python3 -u "${SCRIPT_DIR}/train.py" \
    --ns_tokenizer_type auto \
    --user_ns_tokens 5 \
    --item_ns_tokens 2 \
    --num_queries 2 \
    --ns_groups_json "" \
...
# ---- Alternative config: RankMixer NS tokenizer ----
# To switch to RankMixer, comment out the block above and uncomment below.
#
# USER_DENSE_UE_FIDS="${USER_DENSE_UE_FIDS:-61,87,89,90,91}"
# USER_INT_DENSE_PAIR_FIDS="${USER_INT_DENSE_PAIR_FIDS:-62,63,64,65,66}"
# python3 -u "${SCRIPT_DIR}/train.py" \
