...
def main() -> None:
    ...
    tr_dur = tr_max - tr_min if tr_min is not None else 0
    va_dur = va_max - va_min if va_min is not None else 0
        ...
        for col_name, st in col_stats.items():
            ...
            if st['type'] == 'scalar':
                ...
                if valid_mask.any():
                    ...
                    # Compute per-row lengths from offsets, then filter valid rows
                    # (do NOT index offsets with valid_mask directly — offsets is
                    #  n_rows+1 long and skipping rows would give wrong deltas)
                    all_lens = (offsets[1:] - offsets[:-1]).astype(np.int32)
                    lens = all_lens[valid_mask].tolist()
    ...
    # ── Section 6: Sequence Domain Length Distribution ──────────────────
    log.info(section_header("Section 6: Sequence Domain Length Distribution"))

    # For each domain, collect the effective (non-zero) sequence length of
    # each sample.  We iterate row-group by row-group (same rg_list used
    # throughout the EDA) so we never load the full dataset into memory.
    seq_len_stats = {}  # domain -> np.array of int lengths (accumulated)

    for domain, cfg in sorted(seq_cfg.items()):
        prefix = cfg['prefix']
        features = cfg['features']  # [[fid, vocab_size], ...]
        if not features:
            continue
        # Pick the first sideinfo fid as a proxy for the row's sequence length
        # (all sideinfo columns in a domain share the same per-row length).
        first_fid = features[0][0]
        col_name = f'{prefix}_{first_fid}'
        if col_name not in schema_names:
            log.info(f"  [{domain}] column {col_name} not found in schema, skipping")
            continue

        domain_lengths = []  # accumulate per-row effective lengths
        ...
        for f, rg_idx, n_rows in rg_list:
            ...
            if pa.types.is_list(col.type) or pa.types.is_large_list(col.type):
                ...
                starts = offsets[:-1]
                ends = offsets[1:]
                # Vectorised: cumsum trick to count non-zero elements per row
                # (zeros are padding, consistent with dataset.py truncation)
                nonzero_mask = (values != 0).astype(np.int32)
                cumsum = np.zeros(len(values) + 1, dtype=np.int64)
                cumsum[1:] = np.cumsum(nonzero_mask)
                eff_lens = (cumsum[ends] - cumsum[starts]).astype(np.int32)
                eff_lens[~valid_mask] = 0
                domain_lengths.append(eff_lens)
            else:
                # scalar column — treat effective length as 1
                domain_lengths.append(np.ones(n_rows, dtype=np.int32))

        if domain_lengths:
            seq_len_stats[domain] = np.concatenate(domain_lengths)

    if not seq_len_stats:
        log.info("  (no sequence domains found in schema)")

    # ── per-domain statistics table ──────────────────────────────────────
    thresholds = [64, 128, 256, 512, 1024]
    log.info(f"\n  {'Domain':<10s} {'N':>10s} {'Mean':>8s} {'Median':>8s} "
             f"{'p90':>8s} {'p95':>8s} {'p99':>8s} {'Max':>8s}")
    log.info(f"  {'-'*10} {'-'*10} {'-'*8} {'-'*8} {'-'*8} {'-'*8} {'-'*8} {'-'*8}")
    for domain in sorted(seq_len_stats):
        lens = seq_len_stats[domain].astype(np.int64)
        n = len(lens)
        if n == 0:
            continue
        mean_ = float(lens.mean())
        ...
        max_  = int(lens.max())
        log.info(f"  {domain:<10s} {n:>10,} {mean_:>8.1f} {p50:>8.0f} "
                 f"{p90:>8.0f} {p95:>8.0f} {p99:>8.0f} {max_:>8,}")

    # ── truncation-rate table ─────────────────────────────────────────────
    log.info(f"\n  Truncation rate (% samples with length > threshold):")
    log.info(f"  {'Domain':<10s} " + "  ".join(f">{t:<6d}" for t in thresholds))
    for domain in sorted(seq_len_stats):
        lens = seq_len_stats[domain].astype(np.int64)
        rates = [f"{(lens > t).mean() * 100:>6.2f}%" for t in thresholds]
        log.info(f"  {domain:<10s} " + "  ".join(rates))

    # ── suggested max_len (p99 rounded up to next power of 2) ─────────────
    log.info(f"\n  Suggested max_len (p99 rounded to next power of 2):")
    for domain in sorted(seq_len_stats):
        lens = seq_len_stats[domain].astype(np.int64)
        p99_val = int(np.percentile(lens, 99))
        if p99_val <= 0:
            suggested = 32
        else:
            suggested = 1
            while suggested < p99_val:
                suggested *= 2
        log.info(f"    {domain:<10s}: p99={p99_val}, suggested max_len={suggested}")
