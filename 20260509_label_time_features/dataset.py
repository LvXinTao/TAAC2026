...
class PCVRParquetDataset(IterableDataset):
    ...
    def _load_schema(self, schema_path: str, seq_max_lens: Dict[str, int]) -> None:
        ...
        self._label_time_hour_offset = self.user_int_schema.entries[-2][1]
        self._label_time_dow_offset = self.user_int_schema.entries[-1][1]
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        # ---- label_time derived features ----
        # Extract hour (0-23) and day_of_week (0=Monday, 6=Sunday) from the
        # label_time column (Unix timestamp in seconds). Values are shifted
        # by +1 so that 0 means "unknown/missing" and real values start from 1.
        label_time_col = batch.column(self._col_idx['label_time'])
        label_time_arr = label_time_col.fill_null(0).to_numpy(zero_copy_only=False).astype(np.int64)
        valid_mask = label_time_arr > 0
        hours = (label_time_arr // 3600) % 24
        # 1970-01-01 was Thursday (index 3 if Mon=0), so +4 to align.
        days_of_week = (label_time_arr // 86400 + 4) % 7
        hours[~valid_mask] = 0
        days_of_week[~valid_mask] = 0
        user_int[:, self._label_time_hour_offset] = hours + 1
        user_int[:, self._label_time_dow_offset] = days_of_week + 1
