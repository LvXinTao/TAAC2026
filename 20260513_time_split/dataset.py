...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        row_ranges: Optional[List[Tuple[str, int, int]]] = None,
        ...
        Args:
            ...
            row_group_range: ``(start, end)`` slice of Row Groups; ``None`` to
                use all Row Groups. Ignored when ``row_ranges`` is provided.
            row_ranges: list of ``(file_path, start_row, end_row)`` tuples for
                precise row-level splits (e.g. time-ordered). When provided,
                ``row_group_range`` is ignored and rows are read by range.
        ...
        # Row-level ranges (for time-ordered splits) vs row-group ranges.
        self._row_ranges = row_ranges
        if row_ranges is not None:
            self._rg_list = []  # empty; not used in row_ranges mode
            self.num_rows = sum(end - start for _, start, end in row_ranges)
            # For column index lookup, pick the first file from needed ranges
            # or fall back to first available file.
            needed_files = set(r[0] for r in row_ranges)
            first_file = next(
                (f for f in self._parquet_files if f in needed_files),
                self._parquet_files[0],
            )
            rg_source = f"{len(row_ranges)} row ranges"
        else:
            ...
            first_file = self._parquet_files[0]
            rg_source = f"{len(self._rg_list)} row group(s)"
        ...
        logging.info(
            ...
            f"{rg_source}, {len(self._parquet_files)} file(s), "
    ...
    def _load_schema(self, schema_path: str, seq_max_lens: Dict[str, int]) -> None:
        ...
        # ---- Synthetic features derived from timestamp ----
        # Hour-of-day (0-23) and day-of-week (0-6) are appended as extra
        # user_int features. They are not present in the parquet columns;
        # values are computed in ``_convert_batch`` from the ``timestamp`` column.
        self.user_int_schema.add(201, 1)  # hour
        self.user_int_vocab_sizes.append(24)
        self.user_int_schema.add(202, 1)  # day_of_week
        self.user_int_vocab_sizes.append(7)
        self._timestamp_hour_offset = self.user_int_schema.entries[-2][1]
        self._timestamp_dow_offset = self.user_int_schema.entries[-1][1]
    ...
    def __iter__(self) -> Iterator[Dict[str, Any]]:
        if self._row_ranges is not None:
            yield from self._iter_row_ranges()
        else:
            yield from self._iter_row_groups()

    def _iter_row_groups(self) -> Iterator[Dict[str, Any]]:
        """Row-group-based iteration (original path)."""
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        # Extract hour (0-23) and day_of_week (0=Monday, 6=Sunday) from the
        # timestamp column (Unix timestamp in seconds). Values are shifted
        # by +1 so that 0 means "unknown/missing" and real values start from 1.
        valid_mask = timestamps > 0
        hours = (timestamps // 3600) % 24
        # 1970-01-01 was Thursday (index 3 if Mon=0), so +4 to align.
        days_of_week = (timestamps // 86400 + 4) % 7
        hours[~valid_mask] = 0
        days_of_week[~valid_mask] = 0
        user_int[:, self._timestamp_hour_offset] = hours + 1
        user_int[:, self._timestamp_dow_offset] = days_of_week + 1
...
def get_pcvr_data(
    ...
    clip_vocab: bool = True,
    seq_max_lens: Optional[Dict[str, int]] = None,
    time_split_map_path: Optional[str] = None,
    **kwargs: Any,
) -> Tuple[DataLoader, DataLoader, PCVRParquetDataset]:
    """Create train / valid DataLoaders from raw multi-column Parquet files.

    If ``time_split_map_path`` is provided, the split is read from the JSON
    file generated by ``make_time_split.py`` (time-ordered, precise to the row).
    Otherwise, the split falls back to the last ``valid_ratio`` fraction of Row
    ...
    if time_split_map_path:
        return _get_pcvr_data_time_split(
            data_dir=data_dir,
            ...
            time_split_map_path=time_split_map_path,
        )

    # Fallback: row-group-order split (original behavior).
...
def _get_pcvr_data_time_split(
    ...
    shuffle_train: bool,
    clip_vocab: bool,
    seq_max_lens: Optional[Dict[str, int]],
    time_split_map_path: str,
) -> Tuple[DataLoader, DataLoader, PCVRParquetDataset]:
    """Create train / valid DataLoaders using a time-ordered split map."""
    with open(time_split_map_path) as f:
        split_map = json.load(f)

    train_row_ranges: List[Tuple[str, int, int]] = []
    valid_row_ranges: List[Tuple[str, int, int]] = []
    for fpath, ranges in split_map["files"].items():
        for start, end in ranges.get("train_rows", []):
            train_row_ranges.append((fpath, start, end))
        for start, end in ranges.get("valid_rows", []):
            valid_row_ranges.append((fpath, start, end))

    logging.info(f"Time-ordered split: {split_map['train_rows']:,} train rows "
                 f"(in {len(train_row_ranges)} ranges), "
                 f"{split_map['valid_rows']:,} valid rows "
                 f"(in {len(valid_row_ranges)} ranges)")
