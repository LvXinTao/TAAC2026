...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        self.max_lr_steps: int = max_lr_steps

        # LR schedule: linear warmup → CosineAnnealingLR decay.
        # Phase 1 (step 1..warmup_steps): linear ramp from 0 → lr.
        # Phase 2 (step warmup_steps+1..max_lr_steps): cosine decay lr → 0.
        # If max_lr_steps == 0, LR stays at full value after warmup (no decay).
        ...
        if max_lr_steps > 0 and max_lr_steps > warmup_steps:
            self.cosine_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                self.dense_optimizer,
                T_max=max_lr_steps - warmup_steps,
                eta_min=0,
            )
        ...
        logging.info(f"PCVRHyFormerRankingTrainer loss_type={loss_type}, "
                     ...
                     f"warmup_steps={warmup_steps}, max_lr_steps={max_lr_steps}, "
    ...
    def train(self) -> None:
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            for step, batch in train_pbar:
                ...
                # Apply LR warmup scheduler, then cosine annealing.
                if self.warmup_scheduler is not None and total_step <= self.warmup_steps:
                    self.warmup_scheduler.step()
                elif self.cosine_scheduler is not None and total_step > self.warmup_steps:
                    self.cosine_scheduler.step()

                if self.writer:
                    self.writer.add_scalar('Loss/train', loss, total_step)
                    self.writer.add_scalar('LR/train', self.dense_optimizer.param_groups[0]['lr'], total_step)
