...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        if use_amp:
            logging.info("AMP enabled (torch.cuda.amp)")
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        # Unscale both optimizers so gradients are restored to FP32 scale.
        ...
        # Dense optimizer: use scaler-managed step (AMP-safe).
        self.scaler.step(self.dense_optimizer)
        # Sparse optimizer (Adagrad): step directly with unscaled gradients.
        # Adagrad accumulates squared gradients, so scaled FP16 gradients
        # would overflow the accumulator and produce NaN.
        if self.sparse_optimizer is not None:
