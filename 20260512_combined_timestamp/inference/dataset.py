...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        for domain in self.seq_domains:
            ...
            # Extra slots for synthetic hour + day_of_week features appended to sideinfo
        ...
        for domain in self.seq_domains:
            ...
            for slot, fid in enumerate(sideinfo_fids):
                if fid < 0:
                    continue  # synthetic features have no Parquet column
    ...
    def _load_schema(self, schema_path: str, seq_max_lens: Dict[str, int]) -> None:
        ...
        for domain in self.seq_domains:
            ...
            # Add synthetic features for hour (0-23) and day_of_week (0-6)
            # extracted from timestamp. Use negative FIDs to avoid collision.
            sideinfo.append(-1)  # hour
            sideinfo.append(-2)  # day_of_week
            self.seq_domain_vocab_sizes[domain].extend([24, 7])
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        # ---- timestamp derived features (non-seq) ----
        ...
        for domain in self.seq_domains:
            ...
            # Extract hour and day_of_week from timestamp column, write into
            # the last two synthetic slots (appended after real sideinfo_fids).
            if ts_ci is not None:
                ts_col = batch.column(ts_ci)
                ts_offs = ts_col.offsets.to_numpy()
                ts_vals = ts_col.values.to_numpy()

                hour_slot = len(side_plan)       # first slot after real sideinfo
                dow_slot = len(side_plan) + 1     # one past that

                # Pad timestamps into shape (B, max_len).
                ts_padded = np.zeros((B, max_len), dtype=np.int64)
                ts_valid_len = np.zeros(B, dtype=np.int64)
                for i in range(B):
                    s = int(ts_offs[i])
                    e = int(ts_offs[i + 1])
                    rl = e - s
                ...
                # Convert to hour (0-23) and day_of_week (0-6).
                hours = (ts_padded // 3600) % 24
                days_of_week = (ts_padded // 86400) % 7

                # Write into the synthetic slots.
                for i in range(B):
                    if ts_valid_len[i] > 0:
                        vl = int(ts_valid_len[i])
                        out[i, hour_slot, :vl] = hours[i, :vl]
                        out[i, dow_slot, :vl] = days_of_week[i, :vl]
                        if vl > lengths[i]:
                            lengths[i] = vl
            ...
            # The last two slots (hour, day_of_week) are synthetic — skip OOB.
