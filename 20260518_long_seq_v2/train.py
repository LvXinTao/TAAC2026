...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--warmup_steps', type=int, default=0,
                        help='Number of linear LR warmup steps before reaching target lr '
                             '(0 = no warmup, lr starts at full value)')
    ...
    parser.add_argument('--seq_encoder_type', type=str, default='transformer',
                        ...
                        help='Sequence encoder variant (applied to all domains unless '
                             '--seq_domain_encoder_types overrides per-domain): '
    ...
    parser.add_argument('--seq_domain_encoder_types', type=str, default='',
                        help='Per-domain encoder type override, comma-separated in sorted domain order '
                             '(e.g. "transformer,longer,longer,longer" for seq_a/b/c/d). '
                             'When provided, overrides --seq_encoder_type for all domains. '
                             'Each value must be one of swiglu/transformer/longer.')
    ...
    # User dense UE / int-dense pair features.
    parser.add_argument('--user_dense_ue_fids', type=str, default='',
                        help='Comma-separated user_dense feature IDs for UE projection '
                             '(e.g. 61,87,89,90,91).  Empty = use plain projection.')
    parser.add_argument('--user_int_dense_pair_fids', type=str, default='',
                        help='Comma-separated feature IDs that have both int and dense '
                             'representations for weighted-pair pooling (e.g. 62,63,64,65,66).')
...
def main() -> None:
    ...
    # Resolve seq_encoder_type: per-domain list takes priority over global string.
    # seq_domains is sorted alphabetically: seq_a, seq_b, seq_c, seq_d.
    if args.seq_domain_encoder_types:
        domain_encoder_list = [s.strip() for s in args.seq_domain_encoder_types.split(',')]
        valid_types = {'swiglu', 'transformer', 'longer'}
        for et in domain_encoder_list:
            if et not in valid_types:
                raise ValueError(f"Invalid encoder type '{et}' in --seq_domain_encoder_types. "
                                 f"Must be one of {valid_types}.")
        resolved_seq_encoder_type = domain_encoder_list
        logging.info(f"Per-domain encoder types ({pcvr_dataset.seq_domains}): {resolved_seq_encoder_type}")
    else:
        resolved_seq_encoder_type = args.seq_encoder_type
        logging.info(f"Uniform encoder type for all domains: {resolved_seq_encoder_type}")
    ...
    model_args = {
        ...
        "seq_encoder_type": resolved_seq_encoder_type,
        ...
        "user_dense_ue_fids": args.user_dense_ue_fids,
        "user_int_dense_pair_fids": args.user_int_dense_pair_fids,
    }
    ...
    # Build train_config: start from CLI args, then patch seq_encoder_type with
    # the resolved value (may be a list when --seq_domain_encoder_types is set).
    train_config = vars(args).copy()
    train_config['seq_encoder_type'] = resolved_seq_encoder_type
