...
class MultiSeqHyFormerBlock(nn.Module):
    ...
    def __init__(
        ...
        # Normalize seq_encoder_type to a per-sequence list
        if isinstance(seq_encoder_type, str):
            encoder_types = [seq_encoder_type] * num_sequences
        else:
            if len(seq_encoder_type) != num_sequences:
                raise ValueError(
                    f"seq_encoder_type list length ({len(seq_encoder_type)}) "
                    f"must match num_sequences ({num_sequences})"
                )
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        num_heads: int = 4,
        seq_encoder_type: Union[str, List[str]] = 'transformer',
        hidden_mult: int = 4,
